...
public class SimpleRecipeRenderer extends RecipeRenderer {
    ...
    public static final Comparator<ItemStack> ITEM_STACK_COMPARATOR = (o1, o2) -> {
        if (o1.getItem() == o2.getItem()) {
            ...
            if (compare != 0)
                return compare;
            if (o1.getTag().size() != o2.getTag().size())
                return o1.getTag().size() - o2.getTag().size();
    ...
    public SimpleRecipeRenderer(Supplier<List<List<ItemStack>>> input, Supplier<List<ItemStack>> output) {
        ...
        for(Pair<List<ItemStack>, Integer> pair : a) {
            Optional<Pair<List<ItemStack>, AtomicInteger>> any = newList.stream().filter(pairr -> equalsList(pair.getFirst(), pairr.getFirst())).findAny();
            if (any.isPresent()) {
                any.get().getSecond().addAndGet(pair.getSecond());
            } else
                newList.add(new Pair<>(pair.getFirst(), new AtomicInteger(pair.getSecond())));
        }
        List<List<ItemStack>> b = Lists.newArrayList();
        for(Pair<List<ItemStack>, AtomicInteger> pair : newList)
            b.add(pair.getFirst().stream().map(stack -> {
                ItemStack s = stack.copy();
                s.setCount(pair.getSecond().get());
                return s;
    ...
    public static boolean equalsList(List<ItemStack> list_1, List<ItemStack> list_2) {
        ...
        for(int i = 0; i < stacks_1.size(); i++)
            if (!stacks_1.get(i).isItemEqualIgnoreDurability(stacks_2.get(i)))
                return false;
        return true;
    }
    ...
    public void render(int x, int y, double mouseX, double mouseY, float delta) {
        ...
        for(ItemStackRenderer itemStackRenderer : inputRenderer) {
            ...
            itemStackRenderer.drawTooltip = Minecraft.getInstance().currentScreen instanceof VillagerRecipeViewingScreen;
        ...
        drawTexturedModalRect(xx, yy, 0, 28, 18, 18);
        xx += 18;
        outputRenderer.setBlitOffset(getBlitOffset() + 50);
        outputRenderer.drawTooltip = Minecraft.getInstance().currentScreen instanceof VillagerRecipeViewingScreen;
